...
def handler(event,context):
	...
	s3location = 's3://' + event['Records'][0]['s3']['bucket']['name'] + '/' + event['Records'][0]['object']['key']
	## DEBUG CODE
	print("Got this S3 location: {}".format(s3location))
	return
	## DEBUG CODE
	...
	inputfile_basename = (s3location.split('/')[-1]).split('.')[0]
